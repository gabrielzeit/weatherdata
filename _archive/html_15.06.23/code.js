    ...
    if(analyseChartDataArr != null){
        ...
        for(let p = 0; p < analyseChartDataArr[1].length; p++){
            for(let x = 1; x < analyseChartDataArr.length; x++){
                if(analyseChartDataArr[0].indexOf("date")+1 == x){
                    textToSave += analyseChartDataArr[x][p].slice(0,4) + "-" + analyseChartDataArr[x][p].slice(4,6) + "-" + analyseChartDataArr[x][p].slice(6,8) + ","
                    continue;
                }
                if(analyseChartDataArr[0].indexOf("timehhmmss")+1 == x){
                    textToSave += analyseChartDataArr[x][p].slice(0,2) + ":" + analyseChartDataArr[x][p].slice(2,4) + ":" + analyseChartDataArr[x][p].slice(4,6) + ","
                    continue;
                }

                textToSave += analyseChartDataArr[x][p] + ","
            }
            textToSave = textToSave.slice(0, -1);
            textToSave+="\n";
        }
